...
serve(async (req) => {
  ...
  try {
    ...
    if (isTestToken) {
      console.debug("⚠️ Token de TESTE detectado - modo sandbox ativo");
      return new Response(
        JSON.stringify({
          ok: true,
          mode: "test",
    ...
    console.debug("🔍 Validando token com API do Mercado Pago...");
    ...
    console.debug("✅ Token de PRODUÇÃO válido!");
    console.debug(`🏢 Conta: ${accountData.id} - ${accountData.nickname}`);
